...
if [ $stage -le 1 ]; then
  ...
  for j in $(seq $nj); do cat $dir/xvector.$j.scp; done >$dir/vector.scp || exit 1;
fi

if [ $stage -le 2 ]; then
  # Average the utterance-level xvectors to get lang-level xvectors.
  echo "$0: computing mean of xvectors for each lang"
